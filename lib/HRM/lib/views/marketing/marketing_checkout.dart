...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      // SESSION UID: Priority => login_cus_id, then server_uid, then login uid integer
      ...
      request.fields.addAll({
        'cid': cid,
        'device_id': deviceId,
        'uid': employeeId, // Correct session UID
        'ln': lng,
        'lt': lat,
        'type': '2053',
        ...
        'check_out_location':
            _locationController.text, // Guaranteed param for DB
      });
      ...
      if (_attachments != null && await _attachments!.exists()) {
        request.files.add(
          await http.MultipartFile.fromPath(
            'attachments[]', // Strictly required as list key
            _attachments!.path,
      ...
      debugPrint("Checkout Request Fields: ${request.fields}");
      if (_attachments != null) debugPrint("Attachment: ${_attachments!.path}");
