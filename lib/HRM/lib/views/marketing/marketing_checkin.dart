...
class MarketingCheckInScreen extends StatefulWidget {
  ...
  const MarketingCheckInScreen({super.key, required this.task});
...
class _MarketingCheckInScreenState extends State<MarketingCheckInScreen> {
  ...
  Future<void> _loadEmployeeDetails() async {
    ...
    var uidRaw = prefs.getString('login_cus_id') ?? prefs.get('uid');
    String loginUid = "0";
    if (uidRaw != null) {
  ...
  Future<void> _fetchHistoryFromApi() async {
    ...
    if (employeeTableId == null || employeeTableId == "0") {
      debugPrint("⚠️ Employee ID not set ($employeeTableId), skipping history fetch");
      return;
    }
